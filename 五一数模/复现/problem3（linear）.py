...
from scipy.optimize import dual_annealing  # 模拟退火优化
...
# 读取表3数据
data = pd.read_excel('E:/AAD竞赛/数模/五一建模/2025-51MCM-Problem A/附件(Attachment).xlsx', sheet_name='表3 (Table 3)')
...
first_green_start = 3  # 第一个绿灯开始时间（对应7:06）

# 判断某时刻是否为绿灯
def is_green(t):
    elapsed_time = t - first_green_start  # 计算相对于第一个绿灯开始的时间差
    if elapsed_time < 0:
...
def get_traffic_light_status(t_range):
    return np.array([is_green(t_val) for t_val in t_range])

# 定义支路车流量的参数
def branch_flow(t, params):
    a1, a2, a3, a4, t_break1, t_break2, t_break3, t_break4 = params[:8]
    b1, b2, b3, b4, b5 = params[8:13]
    c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12 = params[13:]

    # 固定支路2的转折点
    t_break6 = 35
    t_break7 = 47

    # 支路1：分段函数，无车流量→线性增长→线性减少→稳定→线性减少至无车流量
    flow1 = np.zeros_like(t, dtype=float)
    flow1[t < t_break1] = 0
    mask_growth = (t >= t_break1) & (t < t_break2)
    flow1[mask_growth] = a1 * (t[mask_growth] - t_break1) + a2
    mask_decrease1 = (t >= t_break2) & (t < t_break3)
    flow1[mask_decrease1] = a3 * (t[mask_decrease1] - t_break2) + (a1 * (t_break2 - t_break1) + a2)
    mask_stable = (t >= t_break3) & (t < t_break4)
    flow1[mask_stable] = a3 * (t_break3 - t_break2) + (a1 * (t_break2 - t_break1) + a2)
    mask_decrease2 = (t >= t_break4)
    flow1[mask_decrease2] = a4 * (t[mask_decrease2] - t_break4) + (a3 * (t_break3 - t_break2) + (a1 * (t_break2 - t_break1) + a2))
    flow1 = np.maximum(flow1, 0)

    # 支路2：分段线性函数
    flow2 = np.zeros_like(t, dtype=float)
    mask_growth1 = (t <= t_break6)
    flow2[mask_growth1] = b1 * t[mask_growth1] + b2
    mask_stable2 = (t > t_break6) & (t <= t_break7)
    flow2[mask_stable2] = b3
    mask_decrease = (t > t_break7)
    flow2[mask_decrease] = b4 * (t[mask_decrease] - t_break7) + b5
    ...
    # 动态生成所有绿灯周期
    num_cycles = (t.max() - first_green_start) // cycle_time + 1  # 计算完整的信号周期数
    cycle_starts = [first_green_start + i * cycle_time for i in range(num_cycles)]  # 生成每个周期的开始时间
    ...
    for cycle_idx, cycle_start in enumerate(cycle_starts):
        ...
        mask_growth = (t >= cycle_start) & (t < growth_end) & mask_green
        flow3[mask_growth] = slope1 * (t[mask_growth] - cycle_start) + intercept1
        # 稳定阶段
        stable_end = cycle_start + 2 * int(green_time / 3)
        mask_stable = (t >= growth_end) & (t < stable_end) & mask_green
        stable_value = slope1 * (growth_end - cycle_start) + intercept1
        flow3[mask_stable] = stable_value
        # 线性减少阶段
        mask_decrease = (t >= stable_end) & (t < cycle_start + green_time) & mask_green
        flow3[mask_decrease] = slope2 * (t[mask_decrease] - stable_end) + stable_value
...
def objective(params):
    ...
    decrease1_start_value = a3 * (t_break2 - t_break2) + (a1 * (t_break2 - t_break1) + a2)
    continuity_penalty1 += 1000 * (growth_end_value - decrease1_start_value)**2
    # 线性减少到稳定
    decrease1_end_value = a3 * (t_break3 - t_break2) + (a1 * (t_break2 - t_break1) + a2)
    stable_value = a3 * (t_break3 - t_break2) + (a1 * (t_break2 - t_break1) + a2)
    continuity_penalty1 += 1000 * (decrease1_end_value - stable_value)**2
    # 稳定到线性减少
    stable_end_value = a3 * (t_break3 - t_break2) + (a1 * (t_break2 - t_break1) + a2)
    decrease2_start_value = a4 * (t_break4 - t_break4) + (a3 * (t_break3 - t_break2) + (a1 * (t_break2 - t_break1) + a2))
    continuity_penalty1 += 1000 * (stable_end_value - decrease2_start_value)**2

    # 支路2连续性约束
    continuity_penalty2 = 0
    # 线性增长到稳定
    growth_end_value2 = b1 * t_break6 + b2
    stable_value2 = b3
    continuity_penalty2 += 1000 * (growth_end_value2 - stable_value2)**2
    # 稳定到线性减少
    stable_end_value2 = b3
    decrease_start_value = b4 * (t_break7 - t_break7) + b5
...
# 设定初始参数
initial_params = [
    0.1, 0.0, -0.1, -0.1, 4.0, 10.0, 20.0, 45.0,
    0.5, 0.0, 20.0, -1.0, 20.0,
    5.0, 0.0, 5.0, -5.0, 5.0, 0.0, 5.0, -5.0, 5.0, 0.0, 5.0, -5.0
]

# 设置参数边界
bounds = [
    (6.0, 7.0), (0.0, 10.0), (-1.0, -0.3), (-2.0, 0.0), (3.0, 5.0), (8.0, 12.0), (18.0, 22.0), (40.0, 50.0),
    (0.1, 2.0), (0.0, 10.0), (30.0, 50.0), (-2.0, -1.0), (30.0, 50.0),
    (0.0, 10.0), (0.0, 40.0), (-10.0, 0.0), (0.0, 40.0),
    (0.0, 10.0), (0.0, 40.0), (-10.0, 0.0), (0.0, 40.0),
    (0.0, 10.0), (0.0, 40.0), (-10.0, 0.0), (0.0, 40.0)
]
...
plt.axvline(x=t_break1, color='g', linestyle='--', alpha=0.5, label=f'支路1转折点')
plt.axvline(x=t_break2, color='g', linestyle='--', alpha=0.5)
plt.axvline(x=t_break3, color='g', linestyle='--', alpha=0.5)
plt.axvline(x=t_break4, color='g', linestyle='--', alpha=0.5)
plt.axvline(x=t_break6, color='m', linestyle='--', alpha=0.5, label=f'支路2转折点')
plt.axvline(x=t_break7, color='m', linestyle='--', alpha=0.5)
for i in range((t.max() - first_green_start) // cycle_time + 1):
    cycle_start = first_green_start + i * cycle_time
    plt.axvspan(cycle_start, cycle_start + green_time, color='green', alpha=0.1)
    if i == 0:
        start = 0
    else:
        start = first_green_start + (i - 1) * cycle_time + green_time
    end = cycle_start
    if start < end:
        plt.axvspan(start, end, color='red', alpha=0.1)
# 处理最后一个周期后的红色区域
last_cycle_end = first_green_start + ((t.max() - first_green_start) // cycle_time) * cycle_time + green_time
if last_cycle_end < t.max():
    plt.axvspan(last_cycle_end, t.max(), color='red', alpha=0.1)
plt.xlabel('时间t (相对于7:00的分钟数/2)')
plt.ylabel('车流量')
plt.title('问题3：各支路车流量')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig('Q-3_主路和支路车流量.png', dpi=300, bbox_inches='tight')
...
plt.xlabel('时间t (相对于7:00的分钟数/2)')
plt.ylabel('车流量')
plt.title('问题3：支路车流量叠加及主路车流量比较')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig('Q-3_支路车流量叠加.png', dpi=300, bbox_inches='tight')

# 灵敏度分析
perturbation = 0.01  # 扰动比例
param_names =['a1', 'b1', 'a2', 'a3', 't1', 't2', 't3', 't4',
              'a4', 'b4', 'c2', 'a5', 'b5',
              'a6.1', 'c3.1', 'a7.1', 'b7.1', 'a6.2', 'c3.2', 'a7.2', 'b7.2', 'a6.3', 'c3.3', 'a7.3', 'b7.3']

sensitivity = []
for i in range(len(params)):
    if params[i] == 0:
        sensitivity.append(0)  # 如果参数为零，灵敏度设为零
        continue
    new_params = params.copy()
    new_params[i] *= (1 + perturbation)
    new_mse = objective(new_params)
    sensitivity.append((new_mse - objective(params)) / (params[i] * perturbation))

# 绘制灵敏度分析柱状图
plt.figure(figsize=(14, 8))
bar_color = (178/255, 221/255, 202/255)
plt.bar(param_names, sensitivity, color=bar_color)
plt.xlabel('参数')
plt.ylabel('灵敏度')
plt.title('问题3：灵敏度分析')
plt.grid(True)
plt.tight_layout()
plt.savefig('Q-3_sensitivity_analysis.png', dpi=300, bbox_inches='tight')

# 保存结果到文件
with open('Q-3_result.txt', 'w', encoding='utf-8') as f:
    f.write("优化结果：\n")
    f.write(f"支路1参数 (a1, a2, a3, a4): ({a1:.4f}, {a2:.4f}, {a3:.4f}, {a4:.4f})\n")
    f.write(f"支路1转折点 (t_break1, t_break2, t_break3, t_break4): ({t_break1:.4f}, {t_break2:.4f}, {t_break3:.4f}, {t_break4:.4f})\n")
    f.write(f"支路2参数 (b1, b2, b3, b4, b5): ({b1:.4f}, {b2:.4f}, {b3:.4f}, {b4:.4f}, {b5:.4f})\n")
    f.write(f"支路2转折点 (t_break6, t_break7): ({t_break6}, {t_break7})\n")
    f.write(f"支路3参数 (c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12): "
    ...
    f.write(f"  当 {t_break2:.2f} <= t < {t_break3:.2f} 时: f1(t) = {a3:.4f}*(t-{t_break2:.2f}) + ({a1:.4f}*({t_break2:.2f}-{t_break1:.2f}) + {a2:.4f})\n")
    f.write(f"  当 {t_break3:.2f} <= t < {t_break4:.2f} 时: f1(t) = {a3:.4f}*({t_break3:.2f}-{t_break2:.2f}) + ({a1:.4f}*({t_break2:.2f}-{t_break1:.2f}) + {a2:.4f})\n")
    f.write(f"  当 t >= {t_break4:.2f} 时: f1(t) = {a4:.4f}*(t-{t_break4:.2f}) + ({a3:.4f}*({t_break3:.2f}-{t_break2:.2f}) + ({a1:.4f}*({t_break2:.2f}-{t_break1:.2f}) + {a2:.4f}))\n")
    f.write("\n支路2:\n")
    f.write(f"  当 t <= {t_break6:.2f} 时: f2(t) = {b1:.4f}*t + {b2:.4f}\n")
    f.write(f"  当 {t_break6:.2f} < t <= {t_break7:.2f} 时: f2(t) = {b3:.4f}\n")
    f.write(f"  当 t > {t_break7:.2f} 时: f2(t) = {b4:.4f}*(t-{t_break7:.2f}) + {b5:.4f}\n")
    f.write("\n支路3:\n")
...
# 打印结果
print("优化结果：")
print(f"支路1参数 (a1, a2, a3, a4): ({a1:.4f}, {a2:.4f}, {a3:.4f}, {a4:.4f})")
print(f"支路1转折点 (t_break1, t_break2, t_break3, t_break4): ({t_break1:.4f}, {t_break2:.4f}, {t_break3:.4f}, {t_break4:.4f})")
print(f"支路2参数 (b1, b2, b3, b4, b5): ({b1:.4f}, {b2:.4f}, {b3:.4f}, {b4:.4f}, {b5:.4f})")
print(f"支路2转折点 (t_break6, t_break7): ({t_break6}, {t_break7})")
...
print(f"  当 {t_break2:.2f} <= t < {t_break3:.2f} 时: f1(t) = {a3:.4f}*(t-{t_break2:.2f}) + ({a1:.4f}*({t_break2:.2f}-{t_break1:.2f}) + {a2:.4f})")
print(f"  当 {t_break3:.2f} <= t < {t_break4:.2f} 时: f1(t) = {a3:.4f}*({t_break3:.2f}-{t_break2:.2f}) + ({a1:.4f}*({t_break2:.2f}-{t_break1:.2f}) + {a2:.4f})")
print(f"  当 t >= {t_break4:.2f} 时: f1(t) = {a4:.4f}*(t-{t_break4:.2f}) + ({a3:.4f}*({t_break3:.2f}-{t_break2:.2f}) + ({a1:.4f}*({t_break2:.2f}-{t_break1:.2f}) + {a2:.4f}))")
print("\n支路2:")
print(f"  当 t <= {t_break6:.2f} 时: f2(t) = {b1:.4f}*t + {b2:.4f}")
print(f"  当 {t_break6:.2f} < t <= {t_break7:.2f} 时: f2(t) = {b3:.4f}")
print(f"  当 t > {t_break7:.2f} 时: f2(t) = {b4:.4f}*(t-{t_break7:.2f}) + {b5:.4f}")
print("\n支路3:")
