...
# 无人机FY3参数
FY3_INITIAL_POS = np.array([6000.0, -3000.0, 700.0])
...
def check_params_validity(params):
    ...
    drone_pos_at_drop = FY3_INITIAL_POS + drone_velocity * t_take
    detonation_point = drone_pos_at_drop + drone_velocity * t_fall
    detonation_point[2] -= 0.5 * G * t_fall**2
...
def generate_valid_solutions(bounds, n_solutions):
    """生成指定数量的有效解。"""
    low_b, high_b = bounds
    valid_solutions = []
    print(f" - 正在为无人机生成 {n_solutions} 组可行解...")
    while len(valid_solutions) < n_solutions:
        random_params = np.random.uniform(low=low_b, high=high_b, size=4)
        if check_params_validity(random_params):
...
if __name__ == "__main__":
    ...
    bounds_low = [np.pi-0.6, 70, 20, 0]
    bounds_high = [np.pi-0.48, 140, 55, 25]#[np.pi/2, 140, 55, 25]#
    bounds = (bounds_low, bounds_high)
    
    # --- 新增：生成并注入高质量初始粒子 ---
    N_PARTICLES = 400
    n_good_particles_target = 300
    print("开始生成高质量初始粒子...")
    
    good_particles = generate_valid_solutions(bounds, n_good_particles_target)
    ...
    random_particles = np.random.uniform(low=bounds_low, high=bounds_high, size=(n_random_particles, 4))
    ...
    # 2. 创建优化器实例
    optimizer = ps.single.GlobalBestPSO(n_particles=N_PARTICLES, dimensions=4, options=options, bounds=bounds, init_pos=init_pos)
