...
def generate_valid_solutions_for_drone_t5(drone_idx, drone_bounds, n_solutions):
    ...
    max_attempts = n_solutions * 2000  # 设置一个尝试上限
...
if __name__ == "__main__":
    ...
    # 1. 为每架无人机生成可行解
    all_bounds_low = [bounds_low_fy1, bounds_low_fy2, bounds_low_fy3, bounds_low_fy4, bounds_low_fy5]
    all_bounds_high = [bounds_high_fy1, bounds_high_fy2, bounds_high_fy3, bounds_high_fy4, bounds_high_fy5]
    
    n_valid_per_drone = 30
    drone_solutions = []
    for i in range(5):
        print(f" - 正在为 FY{i+1} 生成 {n_valid_per_drone} 组可行解...")
        solutions = generate_valid_solutions_for_drone_t5(i, (all_bounds_low[i], all_bounds_high[i]), n_valid_per_drone)
        if solutions.shape[0] == 0:
            raise ValueError(f"无法为无人机 FY{i+1} 找到任何可行解。请检查约束或边界条件。")
