...
def generate_valid_solutions_for_drone_t5_normal(drone_idx, drone_bounds, n_solutions, mean_params, std_dev_divisor=10.0):
    ...
    max_attempts = n_solutions * 4000  # 增加尝试次数以确保能找到足够的有效解
...
if __name__ == "__main__":
    ...
    # 定义分层
    PRIMARY_DRONES = [1, 2]  # 第2层: 优化 FY2, FY3
    SECONDARY_DRONES = [0, 3, 4]

    # 定义粒子生成参数
    N_PRIMARY_SOLUTIONS_PER_DRONE = 100
    N_SECONDARY_SOLUTIONS_PER_DRONE = 50
    N_COMBINATIONS_PER_PRIMARY = 20 # 每个16D主解组合100次
    N_PARTICLES = N_PRIMARY_SOLUTIONS_PER_DRONE * N_COMBINATIONS_PER_PRIMARY

    print("开始生成分层优化高质量初始粒子 (第2层: FY2, FY3)...")
    ...
    drone_solutions = {}
    # 1. 为主要优化无人机生成解 (随机)
    primary_solutions_parts = {}
    for i in PRIMARY_DRONES:
        print(f" - 正在为主要目标 FY{i+1} 生成 {N_PRIMARY_SOLUTIONS_PER_DRONE} 组可行解 (随机)...")
        solutions = generate_valid_solutions_for_drone_t5(i, all_bounds[i], N_PRIMARY_SOLUTIONS_PER_DRONE)
        if solutions.shape[0] < N_PRIMARY_SOLUTIONS_PER_DRONE:
             print(f"警告: 主要目标无人机 FY{i+1} 只找到 {solutions.shape[0]}/{N_PRIMARY_SOLUTIONS_PER_DRONE} 组解。")
        if solutions.shape[0] == 0:
            raise ValueError(f"无法为主要目标无人机 FY{i+1} 找到任何可行解。")
        primary_solutions_parts[i] = solutions
    ...
    # 3. 随机组合主要优化目标的解，生成100组16维解
    print(f" - 正在为主要目标 (FY2, FY3) 组合 {N_PRIMARY_SOLUTIONS_PER_DRONE} 组 16D 解...")
    primary_solutions_combined = []
    solutions_fy2 = primary_solutions_parts[1]
    solutions_fy3 = primary_solutions_parts[2]
    for _ in range(N_PRIMARY_SOLUTIONS_PER_DRONE):
        idx2 = np.random.randint(0, len(solutions_fy2))
        idx3 = np.random.randint(0, len(solutions_fy3))
        combined_sol = np.concatenate([solutions_fy2[idx2], solutions_fy3[idx3]])
        primary_solutions_combined.append(combined_sol)

    # 4. 组合解以生成高质量初始粒子
    print(f" - 正在组合 {N_PARTICLES} 个高质量粒子...")
    init_pos_list = []
    for prim_sol_16d in primary_solutions_combined:
        for _ in range(N_COMBINATIONS_PER_PRIMARY):
            ...
            particle = np.concatenate([
                sec_sol_fy1,
                prim_sol_16d[:8],  # FY2 part
                prim_sol_16d[8:],  # FY3 part
                sec_sol_fy4,
                sec_sol_fy5
            ])
