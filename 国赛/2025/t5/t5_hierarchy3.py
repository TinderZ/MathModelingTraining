...
if __name__ == "__main__":
    ...
    print(f" - 正在为主要目标 (FY1, FY5) 组合 {N_PRIMARY_SOLUTIONS_PER_DRONE} 组 16D 解...")
    primary_solutions_combined = []
    solutions_fy1 = primary_solutions_parts[0]
    solutions_fy5 = primary_solutions_parts[4]
    for _ in range(N_PRIMARY_SOLUTIONS_PER_DRONE):
        idx1 = np.random.randint(0, len(solutions_fy1))
        idx5 = np.random.randint(0, len(solutions_fy5))
        combined_sol = np.concatenate([solutions_fy1[idx1], solutions_fy5[idx5]])
    ...
    for prim_sol_16d in primary_solutions_combined:
        for _ in range(N_COMBINATIONS_PER_PRIMARY):
            ...
            particle = np.concatenate([
                prim_sol_16d[:8],  # FY1 part
                sec_sol_fy2,
                sec_sol_fy3,
                sec_sol_fy4,
                prim_sol_16d[8:],  # FY5 part
            ])
