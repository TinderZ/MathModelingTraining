...
if __name__ == "__main__":
    # 定义12维参数的边界 (从 t4.py 复制)
    bounds_low = np.concatenate([
        [np.pi - 0.2, 139, 0, 2],       # FY1
        [np.pi + 0.09, 70, 0, 6],       # FY2
        [np.pi/2, 70, 0, 4]             # FY3
    ])
    bounds_high = np.concatenate([
        [np.pi, 140, 4, 10],            # FY1
        [np.pi + np.pi/2, 140, 26, 16.9],# FY2
        [np.pi-0.48, 140, 40, 12]       # FY3
    ])

    # --- 以您提供的最优参数为中心进行正态分布随机取点 ---
    
    # 1. 设置最优参数为均值 (mean), 注意角度要从度转换为弧度
    best_params_mean = np.array([
        np.deg2rad(6.3535), 137.8376, 0.0535, 0.7233,       # FY1
        np.deg2rad(267.9904), 108.5223, 6.1151, 6.3542,     # FY2
        np.deg2rad(90.3593), 126.2069, 20.18, 4.3929        # FY3
    ])
    ...
    print("="*60)
    print(f"开始以最优解为中心，进行 {N_ITERATIONS} 次正态分布随机搜索 (T4)...")
    print("="*60)
    start_time = time.time()

    for i in range(N_ITERATIONS):
        # 1. 在最优解附近根据正态分布生成一组新解
        random_params = np.random.normal(loc=best_params_mean, scale=std_devs, size=12)
